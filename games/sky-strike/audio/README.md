...
Original code-generated effects: no MIDI synthesizer, SoundFont, external recording, or runtime synthesis dependency. `synthesis.ts` deterministically generates mono 44.1 kHz PCM; run `node --experimental-strip-types scripts/generate-sky-audio.mjs` from Games to regenerate the 19 bundled WAVs (582,960 bytes).
