import { containmentTransform, roomSpaceTransform, transformPoint } from './space-view';
import {
  type Room,
  type Vec,
  type State,
...
export function portalPose(elapsed: number) {
  ...
  return { active: elapsed >= 0 && elapsed < PORTAL_MS, switched: t >= 0.5,
    phase: smooth(t), opacity: 1 };
...
export function boxTravel(
  ...
  before: State,
  after: State,
  occurrence: 'departure' | 'arrival' = 'departure',
): BoxTravel | undefined {
  ...
  const recursive = transfer.fromRoom === transfer.toRoom;
  const departure = recursive ? occurrence === 'departure' : room === transfer.fromRoom;
  const project = (state: State, position: Vec, source: string, start: boolean) => {
    // Equal room IDs still cross a physical containment edge. An arriving
    // occurrence starts in the child (or parent); a departing one ends there.
    const transform = recursive && start !== departure
      ? containmentTransform(state, transfer.container, start ? !transfer.entering : transfer.entering)
      : roomSpaceTransform(state, source, room, transfer.container);
    const point = position.map((v, i) => i === 1 ? v : v + center - (state.rooms[source]!.size - 1) / 2) as Vec;
    const mapped = transformPoint(point, transform);
    return { position: mapped.map((v, i) => i === 1 ? v : v + (state.rooms[room]!.size - 1) / 2 - center) as Vec, scale: transform.scale };
  };
  const from = project(before, transfer.from, transfer.fromRoom, true);
  const to = project(after, transfer.to, transfer.toRoom, false);
  return { from: from.position, to: to.position, startScale: from.scale, endScale: to.scale,
    entering: transfer.entering, departure };
}
