...
export const REWARD_COPY = {
  zh: { policy:'隐私政策', title:'提示次数', counts:'今日免费 / 广告奖励 / 今日可看广告', rules:'每日免费 1 次，当天有效且优先用；广告奖励可累计', watch:'看广告 · 获得 1 次提示', use:'使用提示', buy:'解锁无限提示 · 无广告', close:'返回', privacy:'广告隐私选项', ready:'每天 1 次免费提示。看完广告再得 1 次', loading:'正在加载广告…', earned:'已获得 1 次提示，已为你保存', cancelled:'广告已关闭，未获得奖励', unavailable:'暂无可用广告，请稍后重试', offline:'网络不可用，请连接后重试', error:'暂时无法完成，请稍后重试', limit:'今日广告次数已用完，明天再来' },
  en: { policy:'Privacy Policy', title:'Hints', counts:'Free today / Ad credits / Ads left today', rules:'Use today’s free hint first. Ad credits carry over', watch:'Watch ad · Get 1 hint', use:'Use hint', buy:'Unlimited hints · No ads', close:'Back', privacy:'Ad privacy options', ready:'1 free hint daily. Watch an ad for 1 more', loading:'Loading ad…', earned:'1 hint earned and saved', cancelled:'Ad closed. No reward earned', unavailable:'No ad available. Please try later', offline:'Offline. Reconnect and try again', error:'Could not complete. Please retry', limit:'Daily ad limit reached. Come back tomorrow' },
  ja: { policy:'プライバシーポリシー', title:'ヒント', counts:'本日の無料 / 広告報酬 / 本日の広告残数', rules:'無料分は当日限り・優先使用。広告報酬は繰り越し', watch:'広告を見てヒントを1回獲得', use:'ヒントを使う', buy:'ヒント無制限 · 広告なし', close:'戻る', privacy:'広告のプライバシー設定', ready:'毎日無料で1回。広告視聴でもう1回', loading:'広告を読み込み中…', earned:'ヒントを1回獲得・保存しました', cancelled:'広告を閉じました。報酬はありません', unavailable:'広告がありません。後でお試しください', offline:'オフラインです。接続後に再試行してください', error:'完了できませんでした。再試行してください', limit:'本日の広告上限です。また明日' },
  fr: { policy:'Politique de confidentialité', title:'Indices', counts:'Gratuit / Crédits pub / Pubs restantes', rules:'Gratuit du jour utilisé en premier. Crédits pub conservés', watch:'Voir une pub · 1 indice', use:'Utiliser un indice', buy:'Indices illimités · Sans pub', close:'Retour', privacy:'Confidentialité des pubs', ready:'1 indice gratuit par jour. Une pub = 1 indice', loading:'Chargement de la publicité…', earned:'1 indice obtenu et sauvegardé', cancelled:'Publicité fermée. Aucune récompense', unavailable:'Aucune publicité. Réessayez plus tard', offline:'Hors ligne. Reconnectez-vous', error:'Impossible de terminer. Réessayez', limit:'Limite atteinte. Revenez demain' },
  de: { policy:'Datenschutzerklärung', title:'Hinweise', counts:'Heute gratis / Werbeguthaben / Werbung übrig', rules:'Gratis zuerst, nur heute. Werbeguthaben bleibt erhalten', watch:'Werbung ansehen · 1 Hinweis', use:'Hinweis nutzen', buy:'Unbegrenzt · Werbefrei', close:'Zurück', privacy:'Werbe-Datenschutz', ready:'1 Hinweis täglich gratis. Werbung = 1 weiterer', loading:'Werbung wird geladen…', earned:'1 Hinweis erhalten und gespeichert', cancelled:'Werbung geschlossen. Keine Belohnung', unavailable:'Keine Werbung verfügbar. Später versuchen', offline:'Offline. Bitte erneut verbinden', error:'Nicht abgeschlossen. Erneut versuchen', limit:'Tageslimit erreicht. Morgen geht es weiter' },
  es: { policy:'Política de privacidad', title:'Pistas', counts:'Gratis hoy / Créditos / Anuncios restantes', rules:'Primero la pista gratis de hoy. Los créditos se acumulan', watch:'Ver anuncio · 1 pista', use:'Usar pista', buy:'Pistas ilimitadas · Sin anuncios', close:'Volver', privacy:'Privacidad de anuncios', ready:'1 pista gratis al día. Un anuncio = 1 más', loading:'Cargando anuncio…', earned:'1 pista obtenida y guardada', cancelled:'Anuncio cerrado. Sin recompensa', unavailable:'No hay anuncios. Inténtalo más tarde', offline:'Sin conexión. Vuelve a conectarte', error:'No se pudo completar. Reintenta', limit:'Límite diario alcanzado. Vuelve mañana' },
} as const;
...
export class CalendarRewardView {
  ...
  private network: CalendarNetworkButtons;
  constructor(private options: { root:GuiRoot; layout:()=>ReturnType<typeof calendarLayout>; language:()=>CalendarLanguage;
    ...
    const label = (id:string,value:()=>string,y:number,size=22) => {
      const element=place(id,new GuiLabel({text:value(),textAlign:'center'}),20,y,760,42);
    ...
    const button=(id:string,value:()=>string,x:number,y:number,w:number,action:()=>void,network=false)=>{
      const element: GuiButton=place(id,new GuiButton({text:'',onClick:network ? calendarNetworkAction(()=>element,action) : action}),x,y,w,58);
      const caption=place(id+'Text',new GuiLabel({text:value(),textAlign:'center',disabled:true}),x+12,y,w-24,58);
      ...
      this.labels.push(()=>{caption.setVisible(this.visible&&!this.network.loading(element));caption.setText(value());caption.setStyle({color:element.disabled?options.root.theme.colors.textMuted:options.root.theme.colors.text});element.markDirty();}); return element;
    };
    ...
    label('rewardRules',()=>this.copy.rules,158,20);
    label('rewardStatus',()=>this.copy[options.rewards.snapshot().phase],198,21);
    this.watch=button('rewardWatch',()=>this.copy.watch,35,254,445,()=>{void options.rewards.watch();},true);
    this.use=button('rewardUse',()=>this.copy.use,500,254,265,options.use);
    this.buy=button('rewardBuy',()=>this.copy.buy,35,330,445,options.buy);
    this.close=button('rewardClose',()=>this.copy.close,500,330,265,options.close);
  ...
  refresh():void {
    ...
    this.watch.disabled=s.busy||s.initializing||s.unlimited||s.adsRemaining===0;
    this.use.disabled=s.busy||(!s.unlimited&&s.free+s.credits===0);
    this.buy.disabled=s.busy||s.unlimited;this.close.disabled=s.busy;
    this.network.set(this.watch,!s.unlimited && (s.initializing || (s.busy && s.operation==='ad' && !s.presenting)));
