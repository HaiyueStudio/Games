...
const RUN_LENGTH = 13;
const INITIAL_DEAL = [6, 6, 6, 6, 5, 5, 5, 5, 5, 5];
...
const PICK_MARGIN_RATIO = 0.035;
const PICK_TOP_RATIO = 0.205;
const PICK_BOTTOM_RATIO = 0.045;
const PICK_CARD_HEIGHT = 198;
const PICK_STACK_GAP = 30;
const GUI_BUTTONS = [
...
function cloneColumns(columns: Card[][]): Card[][] {
  return columns.map(column => column.map(cloneCard));
}

function columnAt(columns: Card[][], index: number): Card[] {
...
function shuffle<T>(items: T[]): T[] {
  for (let i = items.length - 1; i > 0; i--) {
    ...
    const current = requiredItemAt(items, i, 'Spider shuffle items');
    items[i] = requiredItemAt(items, j, 'Spider shuffle items');
    items[j] = current;
  }
  return items;
}
...
new SpiderSolitaire().init(canvas).catch(error => {
  console.error(error);
});
