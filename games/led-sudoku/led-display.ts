...
export const COLORS = { given: '#75e4ef', clue: '#ffc16a', user: '#83ffc1', wrong: '#ff6586', muted: '#162b33' };
/** Seven independent beveled tubes, shared by board and number keypad. */
export function drawDigit(c: CanvasRenderingContext2D, mask: number, x: number, y: number, size: number, color: string, muted = COLORS.muted): void {
  ...
  shapes.forEach((shape, i) => {
    const lit = !!(mask & 1 << i);
    c.fillStyle = lit ? color : muted; c.shadowBlur = 0;
...
export function drawClock(canvas: HTMLCanvasElement, text: string, theme: ThemeId = 'dark'): void {
  ...
  for (const char of text) {
    if (char === ':') {
      ...
      drawDigit(c, char === '0' ? 0x3f : SEGMENTS[Number(char)]!, x, 5, 44, colors.user, colors.tube);
      x += 38;
    }
  }
}
