...
test('attack contact is captured before body push separates a marginal overlap', async () => {
  const air = airBank(box(0, 0, -30, 8, -5)); const fixture = await createFixture({ air }); tick(fixture, {}, true);
  const input = fixture.inputs.push({ P1: ['x'] }); fixture.match.beginTick(input); fixture.match.setKinematics('P1', { position: [-5, 0] }); fixture.match.setKinematics('P2', { position: [5, 0] }); const combat = fixture.combat.step(fixture.match, input, fixture.inputs.history); const result = fixture.match.endTick();
  assert.deepEqual(combat.contacts.map(value => [value.result, value.damage]), [['hit', 100]]); assert.equal(result.state.fighters[1].life, 900); assert(result.state.fighters[1].position[0] > 5, 'body push still resolves after damage');
});

test('air physics cannot render a root fighter below the stage floor', async () => {
  const fallingStates = await parseStates(`[Statedef -1]\n[State -1, fall]\ntype=ChangeState\ntriggerall=ctrl\ntrigger1=command="x"\nvalue=300\n[Statedef 0]\ntype=S\nmovetype=I\nphysics=S\nanim=0\nctrl=1\n[Statedef 300]\ntype=A\nmovetype=I\nphysics=A\nanim=5020\nvelset=0,2\nctrl=0\n`); const fixture = await createFixture({ firstStates: fallingStates }); tick(fixture, {}, true); const frame = tick(fixture, { P1: ['x'] }); const fighter = frame.result.state.fighters[0];
  assert.equal(fighter.position[1], 0); assert(fighter.velocity[1] > 0, 'landing velocity remains visible to the character state on the next tick');
});

test('returning to a controllable grounded state clears stale fall and restores the juggle pool', async () => {
  const recoveryStates = await parseStates(`[Data]\nairjuggle=15\n[Statedef -1]\n[State -1, attack]\ntype=ChangeState\ntriggerall=ctrl\ntrigger1=command="x"\nvalue=200\n[Statedef 0]\ntype=S\nmovetype=I\nphysics=S\nanim=0\nctrl=1\n[Statedef 120]\ntype=S\nmovetype=H\nphysics=N\nanim=120\nctrl=0\n[Statedef 5000]\ntype=S\nmovetype=H\nphysics=N\nanim=5000\nctrl=0\n[Statedef 5020]\ntype=A\nmovetype=H\nphysics=A\nanim=5020\nctrl=0\n[Statedef 5120]\ntype=L\nmovetype=I\nphysics=N\nanim=0\nctrl=0\n[State 5120, keep fall while rising]\ntype=HitFallSet\ntrigger1=time=0\nvalue=1\n[State 5120, stand]\ntype=ChangeState\ntrigger1=time>=1\nvalue=0\n[Statedef 200]\ntype=S\nmovetype=A\nphysics=N\nanim=200\nctrl=0\n[State 200, hit]\ntype=HitDef\ntrigger1=time=0\npersistent=0\nattr=S,NA\ndamage=100\nhitflag=MAF\nground.hittime=1\nground.velocity=0,0\n[State 200, done]\ntype=ChangeState\ntrigger1=time>=1\nvalue=0\n`); const fixture = await createFixture({ firstStates: recoveryStates, secondStates: recoveryStates }); tick(fixture, {}, true);
  const input = fixture.inputs.push({}); fixture.match.beginTick(input); fixture.match.setHitFall('P2', { fall: true }); fixture.match.setJuggleRemaining('P2', 0); fixture.combat.script.enterFighterState(fixture.match, 'P2', 5120, 'P2', {}, false); fixture.combat.step(fixture.match, input, fixture.inputs.history); fixture.match.endTick(); tick(fixture, {});
  const recovered = fixture.match.fighter('P2'); assert.equal(recovered.stateNumber, 0); assert.equal(recovered.hitFall, false); assert.equal(recovered.juggleRemaining, recovered.juggleCapacity);
  const hit = tick(fixture, { P1: ['x'] }); assert.equal(hit.combat.contacts.length, 1); assert.equal(fixture.match.fighter('P2').life, 900);
});
