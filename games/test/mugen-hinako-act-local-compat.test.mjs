...
const directory = fileURLToPath(new URL('../mugen/charactors/HinakoAct/', import.meta.url));
const entryDef = 'HinakoAct.def';

test('local HinakoAct package completes the viewer pipeline', {
  ...
  assert.equal(model.actions.length, 356);
  assert.equal(model.sprites.length, 1_250);
  assert.equal(imported.package.tables.sounds.length, 107);
  assert(imported.viewerAudioCues.length > 149);
  const hitCue = imported.viewerAudioCues.find(cue => cue.group === 300 && cue.item === 21);
  assert(hitCue);
  assert(model.actions.find(action => action.action.number === hitCue.actionNumber)?.audioCues.some(cue => cue.sound.group === 300 && cue.sound.item === 21));
