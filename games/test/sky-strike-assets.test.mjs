...
test('small HUD art uses small alpha textures while Boss art retains preview resolution',()=>{
 for(const [id,limit] of [['gui-life',48],['gui-shield',96],['gui-bomb',160],['gui-pause',160],['fx-rotor',128],['fx-quantum-turret',128],['fx-quantum-preview',256],['boss-quantum-dreadnought',512]]) {
 ...
 for(const e of entries.filter(e=>e.id.startsWith('assets/boss-')))assert.equal(Math.max(e.width,e.height),e.id.includes('boss-twin-')?384:(e.id.includes('boss-miner')||e.id.includes('boss-quantum-dreadnought'))?512:640);
});
