...
const [{ createMugenVfs }, { importMugenCharacter }, { createMugenCharacterModel }, { inferMugenViewerHitAudio }, { buildMugenImportGraph }, { parseMugenExpression }, { MugenInputHistory, MugenLegacyAiInput }, { MugenHeadlessMatch }, { MugenScriptRuntime }, { MugenCombatRuntime }, { placeMugenScreenExplod }] = await Promise.all([
...
test('Petra executable audit has no remaining strict import gap in the legacy G08 profile', { skip: !existsSync(join(directory, entryDef)), timeout: 60_000 }, async () => {
  const imported = await importMugenCharacter(await createMugenVfs(readDirectory(directory)), {
...
test('Petra multipart gauge keeps both player groups intact in the lower screen area', { skip: !existsSync(join(directory, entryDef)), timeout: 30_000 }, async () => {
  ...
  const result = runOpening(imported, 5, () => ({}), [-30, 30]); const gaugeIds = [10000, 10010, 10011, 10012, 10020, 10021, 10022, 10030, 10031, 10032, 10040, 10041, 10042, 10105, 10250, 10300, 10600]; const rows = result.screenExplods.filter(value => gaugeIds.includes(value.explodId));
  assert(rows.some(value => value.rootId === 'P1' && value.position[0] === 55)); assert(rows.some(value => value.rootId === 'P2' && value.position[0] === 265));
  const rules = Object.freeze(gaugeIds.map(explodId => Object.freeze({ explodId, positionScale: Object.freeze([.5, .5]), positionOffset: Object.freeze([0, 110]) }))); const arranged = rows.map(value => placeMugenScreenExplod(value.position, value.explodId, [320, 240], rules)); assert(arranged.every(value => value[1] >= 110 && value[1] <= 226));
});
...
test('G08 Petra native human path enters movement and attack states deterministically', { skip: !existsSync(join(directory, entryDef)), timeout: 30_000 }, async () => {
  ...
  const hurtVoices = first.audio.filter(value => value.fighterId === 'P2' && value.resourceOwner === 'self' && value.group === 101 && value.item >= 0 && value.item <= 8); assert(hurtVoices.length > 0, 'Petra opponent never emitted an authored light/medium/hard hurt voice'); assert(hurtVoices.length <= first.contacts, 'Petra emitted more than one authored hurt voice for a contact');
});

test('Petra human back input walks away without activating a projectile attack', { skip: !existsSync(join(directory, entryDef)), timeout: 30_000 }, async () => {
  const imported = await importMugenCharacter(await createMugenVfs(readDirectory(directory)), { contentRole: 'local-content', entryDef, entryKind: 'character', scriptProfile: 'm09-native-common' });
  const result = runOpening(imported, 35, tick => tick >= 5 && tick < 25 ? { P1: ['left'] } : {}, [-30, 30]); const visited = [...new Set(result.states.map(value => value.p1[0]))];
  assert(result.states.some(value => value.p1[0] === 20), `Petra back input never entered walk state 20; visited ${visited.join(',')}; ai=${result.states.map(value => value.p1Ai.join(',')).join('/')}`);
  assert(Math.min(...result.states.map(value => value.p1X)) < -30, `Petra back input did not move backward; x=${result.states.map(value => value.p1X).join(',')}`);
  assert.equal(Math.max(...result.states.map(value => value.p1Projectiles)), 0, `Petra back input spawned a projectile; visited ${visited.join(',')}`);
});
