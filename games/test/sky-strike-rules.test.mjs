...
test('Sky Strike defines ten regular enemies, four elites, nine boss hulls, and segmented devices', () => {
  assert.equal(ENEMY_DEFINITIONS.length, 26);
  assert.equal(NORMAL_ENEMIES.length, 10);
  assert.equal(ELITE_ENEMIES.length, 4);
  assert.equal(BOSS_ENEMIES.length, 9);
  assert.equal(BOSS_ENEMY.tier, 'boss');
  assert.equal(BOSS_ENEMY.hitPoints, 1_300);
  assert.deepEqual(BOSS_ENEMIES.map(enemy => enemy.bossAttack), ['laser', 'arc-storm', 'gravity-fan', 'carrier-deploy', 'emitter-grid', 'serpent-barrage', 'twin-bubbles', 'twin-bubbles', 'asteroid-grab']);
  assert.equal(new Set(ENEMY_DEFINITIONS.map(enemy => enemy.id)).size, 26);
...
test('level timelines expand grouped spawns and resolve deterministic positions', async () => {
  const levelUrls = [1, 2, 3, 4, 5, 6, 7, 8].map(index => new URL(`../sky-strike/levels/level-0${index}.json`, import.meta.url));
  const levels = await Promise.all(levelUrls.map(async url => JSON.parse(await readFile(url, 'utf8'))));
  assert.deepEqual(levels.map(level => level.bossId), ['dreadnought', 'ion-seraph', 'void-mantis', 'star-carrier', 'helios-prism', 'iron-serpent', 'twin-red', 'ore-reaper']);
  assert.deepEqual(levels.map(level => level.background.top), ['#030617', '#140307', '#0d0418', '#281307', '#031317', '#020d0a', '#10051b', '#0d0c17']);
  ...
  assert.equal(wrapLevelIndex(8, levels.length), 0);
  assert.equal(wrapLevelIndex(-1, levels.length), 7);
