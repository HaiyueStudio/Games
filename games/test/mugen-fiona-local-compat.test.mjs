...
const directory = fileURLToPath(new URL('../mugen/charactors/Fiona/', import.meta.url));
const entryDef = 'Fiona.def';

test('local Fiona package completes the viewer pipeline with legacy AIR sprite arithmetic', {
  ...
  assert.equal(model.actions.length, 1_667);
  assert.equal(model.sprites.length, 1_737);
  assert.equal(imported.package.tables.sounds.length, 178);
  assert(imported.viewerAudioCues.length > 1_000);
  const legacyActions = [168120, 168121].map(actionNumber => model.actions.find(action => action.action.number === actionNumber));
  assert(legacyActions.every(Boolean));
  for (const action of legacyActions) {
    assert.deepEqual(action.action.elements.slice(-2).map(element => [element.spriteGroup, element.spriteItem, element.durationTicks]), [
      [499, 15, 10],
      [499, 15, 30],
    ]);
  }
