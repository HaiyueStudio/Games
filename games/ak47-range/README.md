...
- 右下平时只显示“射击”按钮，按下后展开为半透明瞄准摇杆并连续开火，松手恢复按钮。拖动右摇杆控制人物持枪朝向和 90° 视野；左摇杆独立控制位移，可横向移动或后退射击。按在中心沿当前朝向开火，拖回中心保持最后瞄准方向。子弹从当帧枪口沿瞄准方向发出，保留发射时方向，离开场地、命中角色或掩体后回收。曳光线尺寸缩小至 0.018 × 0.018 × 0.24 米。
...
通过 HTTP 打开 `games/ak47-range/index.html`。浏览器验证保存竖屏/横屏截图与结果到 `.artifacts/ak47-range/twin-stick/`。Native 项目位于工作区的 `Native/examples/ak47-range/`。
