import { failMugen, mugenDiagnostic, type MugenImportDiagnostic } from '../diagnostics';
...
export function parseMugenCommandDocument(document: MugenTextDocument): MugenCommandProgram {
  return parseMugenCommandDocumentWithDiagnostics(document).program;
}

export interface MugenCommandParseResult {
  readonly program: MugenCommandProgram;
  readonly diagnostics: readonly MugenImportDiagnostic[];
}

export function parseMugenCommandDocumentWithDiagnostics(document: MugenTextDocument): MugenCommandParseResult {
  ...
  for (const section of document.sections) {
    ...
    const steps = parseCommandSequence(document, commandToken, remap, diagnostics);
  ...
  const program = Object.freeze({ schemaVersion: 1, revision: 'm08-g08b-command-v1', commands: Object.freeze(commands) }) satisfies MugenCommandProgram;
  return Object.freeze({ program, diagnostics: Object.freeze(diagnostics) });
}

function parseCommandSequence(document: MugenTextDocument, assignment: MugenAssignmentToken, remap: ReadonlyMap<MugenCommandButton, MugenCommandButton>, diagnostics: MugenImportDiagnostic[]): readonly MugenCommandStep[] {
  ...
  return Object.freeze(rawSteps.map(rawStep => {
    ...
    const tokens = rawTokens.map(rawToken => parseCommandToken(document, assignment, rawToken, remap, diagnostics));
...
function parseCommandToken(document: MugenTextDocument, assignment: MugenAssignmentToken, rawToken: string, remap: ReadonlyMap<MugenCommandButton, MugenCommandButton>, diagnostics: MugenImportDiagnostic[]): MugenCommandToken {
  let source = rawToken;
  ...
  if (fourWay && direction === null) {
    // Elecbyte documents $ as direction-only, but accepted WinMUGEN-era
    // characters commonly spell held buttons as /$x. MUGEN treats the stray
    // modifier as redundant; retain the button semantics and make the repair
    // observable rather than rejecting or silently changing the command.
    diagnostics.push(mugenDiagnostic(
      'W_MUGEN_CMD_BUTTON_FOUR_WAY_IGNORED',
      'cmd',
      'warning',
      'ignore',
      `Ignored legacy direction-only $ modifier on MUGEN button token: ${rawToken}.`,
      commandLocation(document, assignment),
      { token: rawToken, normalizedToken: rawToken.replace('$', '') },
    ));
    fourWay = false;
  }
...
function commandLocation(document: MugenTextDocument, assignment: MugenAssignmentToken) { return { canonicalPath: document.canonicalPath, sourceSha256: document.sourceSha256, byteOffset: assignment.valueSpan.startByte, line: assignment.valueSpan.line, column: assignment.valueSpan.column, key: assignment.key } as const; }
