...
export interface MugenResolvedHitOutput {
  ...
  readonly defenderPalette: Readonly<{ time: number; multiply: readonly [number, number, number]; add: readonly [number, number, number]; sineAdd: readonly [number, number, number, number]; invertAll: boolean }>;
}
...
export interface MugenFighterSnapshot {
  ...
  readonly getHitDamage: number;
  readonly getHitGuarded: boolean;
  readonly getHitAnimationType: MugenHitAnimationType;
  readonly getHitAirAnimationType: MugenHitAnimationType;
  readonly getHitGroundType: MugenGroundHitType;
  readonly getHitAirType: MugenGroundHitType;
  readonly getHitVoicePlayed: boolean;
...
interface MutableFighterState {
  ...
  getHitDamage: number;
  getHitGuarded: boolean;
  getHitAnimationType: MugenHitAnimationType;
  getHitAirAnimationType: MugenHitAnimationType;
  getHitGroundType: MugenGroundHitType;
  getHitAirType: MugenGroundHitType;
  getHitVoicePlayed?: boolean;
  hitFall: boolean;
...
export class MugenHeadlessMatch {
  ...
  endTick(): MugenMatchTickResult {
    ...
    for (const fighter of this.#fighters) {
      ...
      if (fighter.hitPauseTicks > 0) fighter.hitPauseTicks -= 1;
        ...
        if (!this.#actionChanged.has(id)) fighter.actionTime = incrementTickCounter(fighter.actionTime, 'fighter actionTime');
  ...
  markFighterStateProcessed(id: string): this { this.#assertOpen(); this.#requireFighter(id); this.#stateProcessedAfterChange.add(id); return this; }
  ...
  setGetHitData(id: string, value: Readonly<{ velocity: readonly [number, number]; yAcceleration: number; damage: number; guarded: boolean; animationType: MugenHitAnimationType; airAnimationType: MugenHitAnimationType; groundType: MugenGroundHitType; airType: MugenGroundHitType; fall: boolean; fallVelocity: readonly [number, number]; fallDamage: number; fallKill: boolean; fallRecover: boolean; fallRecoverTime: number; fallEnvShake: MugenFallEnvShake; downBounce: boolean; hitId: number; attribute: string }>): this { this.#assertOpen(); const fighter = this.#requireFighter(id); const velocity = normalizeVector(value.velocity, `${id}.getHitVelocity`); const fallVelocity = normalizeVector(value.fallVelocity, `${id}.hitFallVelocity`); const fallEnvShake = normalizeFallEnvShake(value.fallEnvShake, `${id}.hitFallEnvShake`); fighter.getHitVelocityX = velocity[0]; fighter.getHitVelocityY = velocity[1]; fighter.getHitYAcceleration = finiteF32(value.yAcceleration, `${id}.getHitYAcceleration`); fighter.getHitDamage = integerRange(value.damage, 0, INT32_MAX, `${id}.getHitDamage`); fighter.getHitGuarded = booleanValue(value.guarded, `${id}.getHitGuarded`); fighter.getHitAnimationType = enumValue(value.animationType, HIT_ANIMATION_TYPES, `${id}.getHitAnimationType`); fighter.getHitAirAnimationType = enumValue(value.airAnimationType, HIT_ANIMATION_TYPES, `${id}.getHitAirAnimationType`); fighter.getHitGroundType = enumValue(value.groundType, GROUND_HIT_TYPES, `${id}.getHitGroundType`); fighter.getHitAirType = enumValue(value.airType, GROUND_HIT_TYPES, `${id}.getHitAirType`); fighter.getHitVoicePlayed = false; fighter.hitFall = booleanValue(value.fall, `${id}.hitFall`); fighter.hitFallVelocityX = fallVelocity[0]; fighter.hitFallVelocityY = fallVelocity[1]; fighter.hitFallDamage = integerRange(value.fallDamage, 0, INT32_MAX, `${id}.hitFallDamage`); fighter.hitFallKill = booleanValue(value.fallKill, `${id}.hitFallKill`); fighter.hitFallRecover = booleanValue(value.fallRecover, `${id}.hitFallRecover`); fighter.hitFallRecoverTime = integerRange(value.fallRecoverTime, 0, 3_600, `${id}.hitFallRecoverTime`); [fighter.hitFallEnvShakeTime, fighter.hitFallEnvShakeFrequency, fighter.hitFallEnvShakeAmplitude, fighter.hitFallEnvShakePhase] = fallEnvShake; fighter.hitDownBounce = booleanValue(value.downBounce, `${id}.hitDownBounce`); fighter.hitElapsedTicks = 0; fighter.lastHitId = int32(value.hitId, `${id}.lastHitId`); if (!HIT_ATTRIBUTE_KEYS.has(value.attribute)) throw new TypeError(`${id}.lastHitAttribute is invalid.`); fighter.lastHitAttribute = value.attribute; return this; }
  claimGetHitVoice(id: string): boolean { this.#assertOpen(); const fighter = this.#requireFighter(id); if (fighter.getHitVoicePlayed) return false; fighter.getHitVoicePlayed = true; return true; }
  ...
  reset(): this {
    ...
    this.#openTick = false; this.#inputHash = ''; this.#stateChanged.clear(); this.#actionChanged.clear(); this.#stateProcessedAfterChange.clear(); this.#stunChanged.clear(); this.#contactChanged.clear(); this.#phaseChanged = false;
  ...
  #resetFighterForRound(fighter: MutableFighterState): void {
    ...
    fighter.stateGeneration = 0; fighter.stateDataOwnerId = fighter.definition.id; fighter.animationOwnerId = fighter.definition.id; fighter.animationElement = 1; fighter.spritePriority = 0; fighter.positionFrozen = false; fighter.widthEdgeFront = 0; fighter.widthEdgeBack = 0; fighter.widthPlayerFront = 0; fighter.widthPlayerBack = 0; fighter.juggleCost = 0; fighter.juggleCapacity = 15; fighter.juggleRemaining = 15; fighter.attackMultiplier = 1; fighter.defenseMultiplier = 1; fighter.playerPushEnabled = true; fighter.getHitVelocityX = 0; fighter.getHitVelocityY = 0; fighter.getHitYAcceleration = 0; fighter.getHitDamage = 0; fighter.getHitGuarded = false; fighter.getHitAnimationType = 'light'; fighter.getHitAirAnimationType = 'light'; fighter.getHitGroundType = 'high'; fighter.getHitAirType = 'high'; fighter.getHitVoicePlayed = false; fighter.hitFall = false; fighter.hitFallVelocityX = 0; fighter.hitFallVelocityY = -4.5; fighter.hitFallDamage = 0; fighter.hitFallKill = true; fighter.hitFallRecover = true; fighter.hitFallRecoverTime = 4; fighter.hitFallEnvShakeTime = 0; fighter.hitFallEnvShakeFrequency = 60; fighter.hitFallEnvShakeAmplitude = -4; fighter.hitFallEnvShakePhase = 0; fighter.hitDownBounce = false; fighter.hitElapsedTicks = 0; fighter.lastHitId = 0; fighter.lastHitAttribute = ''; fighter.hitPauseTicks = 0; fighter.stunTicks = 0; fighter.stunKind = null; fighter.moveContact = 'none'; fighter.moveContactTime = 0; fighter.hitCount = 0; fighter.activeHitDefinition = null; fighter.hitAttributeSlots = [null, null]; fighter.targets = []; fighter.targetBinding = null; fighter.hitOverrides = Array.from({ length: 8 }, () => null); fighter.activeReversalDefinition = null;
