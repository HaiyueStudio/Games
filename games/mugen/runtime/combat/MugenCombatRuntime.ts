...
export class MugenCombatRuntime {
  ...
  #constrainFighters(match: MugenHeadlessMatch, updateCamera: boolean): void {
    ...
    for (const id of this.#order) {
      ...
      const movingFartherOutside = x < fighter.position[0] && fighter.velocity[0] > 0 || x > fighter.position[0] && fighter.velocity[0] < 0; const velocityX = movingFartherOutside || (x === bounds[0] && fighter.velocity[0] < 0) || (x === bounds[1] && fighter.velocity[0] > 0) || (x === this.#stageBounds[0] && fighter.velocity[0] < 0) || (x === this.#stageBounds[1] && fighter.velocity[0] > 0) ? 0 : fighter.velocity[0];
  ...
  #helperAnimation(match: MugenHeadlessMatch, helper: MugenHelperEntitySnapshot): MugenAirSnapshot | null { if (helper.stateDefinitionPending) return null; const config = this.#fighters.get(helper.rootId); const action = config?.actions.get(helper.actionNumber); if (!config || !action) return null; return evaluateMugenAirAction(action, helper.actionTime, { x: helper.position[0], y: helper.position[1], facing: helper.facing, coordinateScale: config.coordinateScale }); }
  ...
  #applyHitOutput(match: MugenHeadlessMatch, attackerRootId: string, attackerEntityId: string, attackerPosition: readonly [number, number], attackerFacing: -1 | 1, defenderEntityId: string, defenderPosition: readonly [number, number], guarded: boolean, hitDef: MugenActiveHitDefinition): void {
    ...
    if (guarded) return;
    const palette = output.defenderPalette; if (palette.time !== 0) this.script.outputs.setPalette(defenderEntityId, { remainingTicks: palette.time, elapsedTicks: 0, add: palette.add, multiply: palette.multiply, sineAdd: palette.sineAdd, invertAll: palette.invertAll, color: 256 });
  ...
  #resolveTimeOver(match: MugenHeadlessMatch): void {
    const first = match.fighter(this.#order[0]); const second = match.fighter(this.#order[1]); const winnerId = first.life === second.life ? null : first.life > second.life ? first.id : second.id;
    match.resolveRound(winnerId, winnerId === null ? 'draw' : 'time-over');
    for (const fighterId of this.#order) {
      const resultState = winnerId === null ? 175 : fighterId === winnerId ? 180 : 170;
      if (this.script.hasFighterState(fighterId, resultState)) this.script.enterFighterState(match, fighterId, resultState, fighterId, {}, false);
    }
  }
}
...
function resolveImmediateReactionAction(config: NormalizedFighterConfig, stateType: MugenFighterSnapshot['stateType'], hitDef: MugenActiveHitDefinition, guarded: boolean, fallback: number): number {
  if (guarded) return config.guardAction;
  const animationType = stateType === 'A' ? hitDef.airAnimationType : hitDef.animationType;
  const typeIndex = ({ light: 0, medium: 1, hard: 2, back: 3, up: 4, diagup: 5 } as const)[animationType];
  let candidate: number;
  if (typeIndex === 3) candidate = 5030;
  else if (typeIndex >= 4) candidate = 5047 + typeIndex;
  else if (stateType === 'C') candidate = 5020 + typeIndex;
  else {
    const hitType = stateType === 'A' ? hitDef.airHitType : hitDef.groundHitType;
    candidate = (hitType === 'high' ? 5000 : 5010) + typeIndex;
  }
  return config.actions.has(candidate) ? candidate : fallback;
}
