...
export class MugenScriptRuntime {
  ...
  #execute(controller: MugenStateController, program: Readonly<{ states: ReadonlyMap<number, MugenStateDefinition>; gravity: number }>, context: EvaluationContext): boolean {
    ...
    switch (controller.type) {
      ...
      case 'pos-freeze': commit(match, { kind: 'position-freeze', fighterId: id, value: p.value !== undefined ? truthy(evaluate(p.value, context)) : (p.x === undefined || truthy(evaluate(p.x, context))) || (p.y === undefined || truthy(evaluate(p.y, context))) }); return false;
