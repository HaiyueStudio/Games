...
export function carouselOffset(index: number, position: number): number {
  return ((index - position + 1.5) % 3 + 3) % 3 - 1.5;
}
...
export function carouselPose(index: number, position: number, width: number, height: number) {
  const d = carouselOffset(index, position), m = carouselMetrics(width, height);
...
export function projectCard(index: number, position: number, width: number, height: number, u: number, v: number) {
  const p = carouselPose(index, position, width, height), x = (u - 0.5) * p.cardWidth, y = (v - 0.5) * p.cardHeight;
  const w = 1 + (p.depth - Math.sin(p.yaw) * x) / p.focal;
  return { x: width / 2 + (p.d * p.spacing + Math.cos(p.yaw) * x) / w, y: height / 2 + (y + p.shiftY) / w };
}
export function cardUv(index: number, position: number, width: number, height: number, x: number, y: number) {
  const p = carouselPose(index, position, width, height), qx = x - width / 2;
...
export function hitCarousel(position: number, width: number, height: number, x: number, y: number): number {
  if (x < 0 || y < 0 || x > width || y > height) return -1;
  for (const index of [0, 1, 2].sort((a, b) => Math.abs(carouselOffset(a, position)) - Math.abs(carouselOffset(b, position)))) {
    const uv = cardUv(index, position, width, height, x, y);
    if (carouselPose(index, position, width, height).alpha > 0.1 && uv.u > 0.02 && uv.u < 0.98 && uv.v > 0.035 && uv.v < 0.965) return index;
  }
  return -1;
}
