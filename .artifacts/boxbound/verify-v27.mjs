...
for(const arg of process.argv.slice(2)){
 ...
 if(result.visualCapture?.pngBase64){writeFileSync(out+`v27-${view}-${width}.png`,Buffer.from(result.visualCapture.pngBase64,'base64'));delete result.visualCapture.pngBase64;}
 ...
 writeFileSync(out+`v27-${view}-${width}.json`,JSON.stringify(result,null,2));
 console.log(view,width,result.status,result.cases?.length,result.errors);
 assert.equal(result.status,'passed');assert.equal(result.browserDiagnostics.unclassifiedFailureCount,0);assert.equal(result.cases.length,['chapters-eight','face-top'].includes(view)?48:view.startsWith('camera-')?42:view.startsWith('recursive-crate')?44:['outer-render','outer-push'].includes(view)?40:view.startsWith('recursive-exit')?41:view.startsWith('exit-')?41:view.startsWith('actor-')?39:view==='box-identity'?45:view==='completion-return'?38:view==='completion-outro'?35:view==='outer-context'?36:view==='parabox-recursive'?51:view==='parabox-nested'?43:view.startsWith('parabox-')?42:['undo-step','undo-box','outer-wall'].includes(view)?37:view.startsWith('box-')?40:['transition','climb','descent'].includes(view)?30:29);
}
